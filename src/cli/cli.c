...
static const char CBM_DEFAULT_MCP_SERVER_NAME[] = "application-memory-mcp";
static const char CBM_ANALYSIS_MCP_SERVER_NAME[] = "application-memory-analysis";
static const char CBM_SCOUT_MCP_SERVER_NAME[] = "application-memory-scout";
...
#ifdef _WIN32
    snprintf(binary_path, binary_path_size, "%s/.local/bin/application-memory-mcp.exe", home);
#else
    snprintf(binary_path, binary_path_size, "%s/.local/bin/application-memory-mcp", home);
#endif
}
...
static bool cbm_detect_self_path(char *buf, size_t buf_sz, const char *home) {
    ...
    if (!buf[0]) {
#ifdef _WIN32
        snprintf(buf, buf_sz, "%s/.local/bin/application-memory-mcp.exe", home);
#else
        snprintf(buf, buf_sz, "%s/.local/bin/application-memory-mcp", home);
#endif
    }
    return exact;
}
...
int cbm_cmd_install(int argc, char **argv) {
...
#ifdef _WIN32
    int target_length =
        snprintf(bin_target, sizeof(bin_target), "%s/application-memory-mcp.exe", bin_dir);
#else
    int target_length = snprintf(bin_target, sizeof(bin_target), "%s/application-memory-mcp", bin_dir);
#endif
    ...
    printf("application-memory-mcp install %s\n\n", CBM_VERSION);
    ...
    if (!dry_run && prepare_binary) {
        ...
        if (prepare_out_of_line) {
            ...
            int candidate_length = snprintf(prepared_candidate, sizeof(prepared_candidate),
                                            "%s/application-memory-mcp.exe", prepared_dir);
#else
            int candidate_length = snprintf(prepared_candidate, sizeof(prepared_candidate),
                                            "%s/application-memory-mcp", prepared_dir);
#endif
