...
static const tool_def_t TOOLS[] = {
    ...
    {"index_database", "Index database",
     "One-time database introspection: connects once, extracts schema (tables, columns, PKs, FKs, "
     "indexes), detects application domain, samples data coherently, and stores everything locally "
     "in the project graph. Supports postgres, mysql, sqlite, mongodb. After indexing, all other "
     "database tools work offline from the cached graph.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"connection_string\":{\"type\":\"string\",\"description\":\"DB URI (postgres://..., "
     "mysql://..., mongodb://..., /path/to.db)\"},"
     "\"dialect\":{\"type\":\"string\",\"enum\":[\"postgres\",\"mysql\",\"sqlite\",\"mongodb\"]},"
     "\"source_env\":{\"type\":\"string\",\"enum\":[\"production\",\"staging\",\"local\","
     "\"development\"],\"default\":\"local\"},"
     "\"encrypt_samples\":{\"type\":\"boolean\",\"default\":false},"
     "\"sample_depth\":{\"type\":\"integer\",\"default\":3},"
     "\"sample_seed_table\":{\"type\":\"string\"}},"
     "\"required\":[\"project\",\"dialect\"]}"},

    {"get_database_schema", "Get database schema",
     "Return cached database schema: tables, columns, PKs, FKs, indexes, comments. "
     "Works entirely from the local graph — no live DB connection needed.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"table_pattern\":{\"type\":\"string\",\"description\":\"Regex filter on table names\"},"
     "\"schema_name\":{\"type\":\"string\"},"
     "\"detail\":{\"type\":\"string\",\"enum\":[\"tables_only\",\"columns\",\"full\"],"
     "\"default\":\"columns\"},"
     "\"format\":{\"type\":\"string\",\"enum\":[\"tree\",\"json\"],\"default\":\"tree\"}},"
     "\"required\":[\"project\"]}"},

    {"generate_db_erd", "Generate database ERD",
     "Generate a Mermaid ER diagram from the cached database graph. "
     "No live DB connection needed.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"tables\":{\"type\":\"array\",\"items\":{\"type\":\"string\"},"
     "\"description\":\"Specific tables; omit for all\"},"
     "\"detail\":{\"type\":\"string\",\"enum\":[\"keys_only\",\"columns\",\"full\"],"
     "\"default\":\"columns\"},"
     "\"max_tables\":{\"type\":\"integer\",\"default\":30},"
     "\"format\":{\"type\":\"string\",\"enum\":[\"mermaid\",\"json\"],\"default\":\"mermaid\"}},"
     "\"required\":[\"project\"]}"},

    {"get_table_data", "Get table data",
     "Retrieve cached sample data or full lookup/config table data. "
     "No live DB connection needed.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"table_name\":{\"type\":\"string\"},"
     "\"mode\":{\"type\":\"string\",\"enum\":[\"sample\",\"full\"],\"default\":\"sample\"},"
     "\"format\":{\"type\":\"string\",\"enum\":[\"tree\",\"json\"],\"default\":\"tree\"}},"
     "\"required\":[\"project\",\"table_name\"]}"},

    {"search_database_graph", "Search database graph",
     "Search across database entities (tables, columns, FKs, indexes) by name pattern "
     "or keyword. Works from cached graph.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"query\":{\"type\":\"string\",\"description\":\"Search keyword\"},"
     "\"name_pattern\":{\"type\":\"string\",\"description\":\"Regex on table/column names\"},"
     "\"entity_type\":{\"type\":\"string\",\"enum\":[\"table\",\"column\",\"index\","
     "\"foreign_key\"]},"
     "\"format\":{\"type\":\"string\",\"enum\":[\"tree\",\"json\"],\"default\":\"tree\"},"
...
static const tool_annotation_def_t TOOL_ANNOTATIONS[] = {
    ...
    {"index_database", false, false, true, true},
    {"get_database_schema", true, false, true, false},
...
static char *cbm_mcp_initialize_response_for_profile(const char *params_json,
    ...
    yyjson_mut_obj_add_str(doc, impl, "name", "application-memory-mcp");
 ...
 *
 * These handlers implement the Full Stack Application Memory MCP database
 * knowledge graph tools. They follow the same pattern as existing handlers:
 * parse args → resolve store → operate → return cbm_mcp_text_result. */

/* Forward declaration for case-insensitive search used by search_database_graph. */
static const char *strcasestr_compat(const char *haystack, const char *needle);

/* ── handle_index_database ────────────────────────────────────── */

static char *handle_index_database(cbm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *dialect_str = cbm_mcp_get_string_arg(args, "dialect");
    char *conn_str = cbm_mcp_get_string_arg(args, "connection_string");
    char *source_env_str = cbm_mcp_get_string_arg(args, "source_env");
    char *seed_table = cbm_mcp_get_string_arg(args, "sample_seed_table");
    bool encrypt = cbm_mcp_get_bool_arg(args, "encrypt_samples");
    int sample_depth = cbm_mcp_get_int_arg(args, "sample_depth", 3);
    ...
    if (!project) {
        ...
        free(source_env_str);
        free(seed_table);
        return cbm_mcp_text_result("missing required argument: project", true);
    }
    if (!dialect_str) {
        free(project);
        free(conn_str);
        free(source_env_str);
        free(seed_table);
        return cbm_mcp_text_result("missing required argument: dialect", true);
    }

    /* Parse dialect. */
    cbm_db_dialect_t dialect;
    if (cbm_db_parse_dialect(dialect_str, &dialect) != 0) {
        char err[CBM_SZ_256];
        snprintf(err, sizeof(err), "unknown dialect: %s (expected: postgres, mysql, sqlite, mongodb)",
                 dialect_str);
        free(project);
        free(dialect_str);
        free(conn_str);
    ...
    /* Resolve connection string: tool param → env var → error. */
    if (!conn_str || conn_str[0] == '\0') {
        free(conn_str);
        conn_str = NULL;
        /* Try environment variables. */
        const char *env_keys[] = {"DATABASE_URL", "PG_URI", "MYSQL_URI", "MONGO_URI", "SQLITE_PATH"};
        for (int i = 0; i < (int)(sizeof(env_keys) / sizeof(env_keys[0])); i++) {
            const char *val = getenv(env_keys[i]);
            if (val && val[0]) {
                conn_str = heap_strdup(val);
                break;
            }
        }
    }
    ...
    if (!conn_str) {
        ...
        free(seed_table);
        return cbm_mcp_text_result(
            "no connection string provided. Pass connection_string parameter or set "
            "DATABASE_URL / PG_URI / MYSQL_URI / MONGO_URI / SQLITE_PATH env var",
            true);
    }

    /* Parse source env. */
    cbm_db_source_env_t source_env = CBM_DB_ENV_LOCAL;
    if (source_env_str) {
        cbm_db_parse_source_env(source_env_str, &source_env);
        free(source_env_str);
    }
    ...
    opts.source_env = source_env;
    opts.encrypt_samples = encrypt;
    opts.sample_depth = sample_depth;
    opts.sample_seed_table = seed_table;

    /* Run introspection. */
    cbm_db_result_t *result = cbm_db_introspect(&opts);
    free(conn_str);
    free(seed_table);

    if (!result) {
        free(project);
        return cbm_mcp_text_result("database introspection failed: allocation error", true);
    }
    if (result->error) {
        char *err_result = cbm_mcp_text_result(result->error, true);
        cbm_db_result_free(result);
        free(project);
        return err_result;
    }

    /* Store results in the project graph as nodes and edges. */
    cbm_store_t *store = resolve_store(srv, project);
    if (!store) {
        /* Try to create the store for this project. */
        store = cbm_store_open(project);
        if (store) {
            cbm_store_upsert_project(store, project, ".");
            if (srv) {
                srv->store = store;
                srv->owns_store = true;
            }
    ...
    if (!store) {
        cbm_db_result_free(result);
        free(project);
        return cbm_mcp_text_result("failed to open project store for database indexing", true);
    }

    /* Count total objects for the summary. */
    int total_tables = 0;
    int total_columns = 0;
    int total_fks = 0;
    int total_indexes = 0;
    ...
    cbm_node_t db_node = {0};
    db_node.project = project;
    db_node.name = result->database_name ? result->database_name : "unknown";
    db_node.label = "Database";
    char db_qn[CBM_SZ_512];
    snprintf(db_qn, sizeof(db_qn), "db:%s", db_node.name);
    db_node.qualified_name = db_qn;
    char db_props[CBM_SZ_2K];
    snprintf(db_props, sizeof(db_props),
             "{\"dialect\":\"%s\",\"version\":\"%s\",\"host\":\"%s\","
             "\"source_env\":\"%s\",\"domain\":\"%s\",\"seed_table\":\"%s\"}",
             cbm_db_dialect_label(result->dialect),
             result->version ? result->version : "",
             result->host ? result->host : "",
             cbm_db_source_env_label(result->source_env),
             result->domain_label ? result->domain_label : "Generic",
             result->seed_table ? result->seed_table : "");
    db_node.properties_json = db_props;
    int64_t db_id = cbm_store_upsert_node(store, &db_node);

    for (int s = 0; s < result->schema_count; s++) {
        cbm_db_schema_t *schema = &result->schemas[s];

        /* Create Schema node. */
        cbm_node_t sch_node = {0};
        sch_node.project = project;
        sch_node.name = schema->name;
        sch_node.label = "Schema";
        char sch_qn[CBM_SZ_512];
        snprintf(sch_qn, sizeof(sch_qn), "db:%s.%s", db_node.name, schema->name);
        sch_node.qualified_name = sch_qn;
        int64_t sch_id = cbm_store_upsert_node(store, &sch_node);

        /* CONTAINS_SCHEMA edge. */
        if (db_id > 0 && sch_id > 0) {
            cbm_edge_t e = {0};
            e.project = project;
            e.source_id = db_id;
            e.target_id = sch_id;
            e.type = "CONTAINS_SCHEMA";
            cbm_store_insert_edge(store, &e);
        }

        for (int t = 0; t < schema->table_count; t++) {
            cbm_db_table_t *tbl = &schema->tables[t];
            total_tables++;

            /* Create Table node. */
            cbm_node_t tbl_node = {0};
            tbl_node.project = project;
            tbl_node.name = tbl->name;
            tbl_node.label = tbl->is_view ? "View" : "Table";
            char tbl_qn[CBM_SZ_1K];
            snprintf(tbl_qn, sizeof(tbl_qn), "db:%s.%s.%s",
                     db_node.name, schema->name, tbl->name);
            tbl_node.qualified_name = tbl_qn;
            char tbl_props[CBM_SZ_1K];
            snprintf(tbl_props, sizeof(tbl_props),
                     "{\"table_type\":\"%s\",\"row_count\":%lld,\"is_lookup\":%s}",
                     tbl->table_type ? tbl->table_type : "",
                     (long long)tbl->row_count_estimate,
                     tbl->is_lookup ? "true" : "false");
            tbl_node.properties_json = tbl_props;
            int64_t tbl_id = cbm_store_upsert_node(store, &tbl_node);

            /* CONTAINS_TABLE edge. */
            if (sch_id > 0 && tbl_id > 0) {
                cbm_edge_t e = {0};
                e.project = project;
                e.source_id = sch_id;
                e.target_id = tbl_id;
                e.type = "CONTAINS_TABLE";
                cbm_store_insert_edge(store, &e);
            }

            /* Column nodes + HAS_COLUMN edges. */
            for (int c = 0; c < tbl->column_count; c++) {
                cbm_db_column_t *col = &tbl->columns[c];
                total_columns++;

                cbm_node_t col_node = {0};
                col_node.project = project;
                col_node.name = col->name;
                col_node.label = "Column";
                char col_qn[CBM_SZ_1K];
                snprintf(col_qn, sizeof(col_qn), "db:%s.%s.%s.%s",
                         db_node.name, schema->name, tbl->name, col->name);
                col_node.qualified_name = col_qn;
                char col_props[CBM_SZ_1K];
                snprintf(col_props, sizeof(col_props),
                         "{\"data_type\":\"%s\",\"ordinal\":%d,\"is_pk\":%s,"
                         "\"is_nullable\":%s,\"is_unique\":%s,\"is_fk\":%s}",
                         col->data_type ? col->data_type : "",
                         col->ordinal,
                         col->is_pk ? "true" : "false",
                         col->is_nullable ? "true" : "false",
                         col->is_unique ? "true" : "false",
                         col->is_fk ? "true" : "false");
                col_node.properties_json = col_props;
                int64_t col_id = cbm_store_upsert_node(store, &col_node);

                if (tbl_id > 0 && col_id > 0) {
                    cbm_edge_t e = {0};
                    e.project = project;
                    e.source_id = tbl_id;
                    e.target_id = col_id;
            ...
            /* Index nodes + HAS_INDEX edges. */
            for (int i = 0; i < tbl->index_count; i++) {
                cbm_db_index_t *idx = &tbl->indexes[i];
                total_indexes++;

                cbm_node_t idx_node = {0};
                idx_node.project = project;
                idx_node.name = idx->name;
                idx_node.label = "Index";
                char idx_qn[CBM_SZ_1K];
                snprintf(idx_qn, sizeof(idx_qn), "db:%s.%s.%s.idx:%s",
                         db_node.name, schema->name, tbl->name, idx->name);
                idx_node.qualified_name = idx_qn;

                /* Build columns_csv for index properties. */
                char cols_csv[CBM_SZ_1K] = "";
                int off = 0;
                for (int ic = 0; ic < idx->column_count && off < (int)sizeof(cols_csv) - 1; ic++) {
                    if (ic > 0) {
                        off += snprintf(cols_csv + off, sizeof(cols_csv) - (size_t)off, ",");
                    }
                    off += snprintf(cols_csv + off, sizeof(cols_csv) - (size_t)off, "%s",
                                    idx->columns[ic] ? idx->columns[ic] : "");
                }

                char idx_props[CBM_SZ_1K];
                snprintf(idx_props, sizeof(idx_props),
                         "{\"index_type\":\"%s\",\"is_unique\":%s,"
                         "\"is_primary\":%s,\"columns_csv\":\"%s\"}",
                         idx->index_type ? idx->index_type : "",
                         idx->is_unique ? "true" : "false",
                         idx->is_primary ? "true" : "false",
                         cols_csv);
                idx_node.properties_json = idx_props;
                int64_t idx_id = cbm_store_upsert_node(store, &idx_node);

                if (tbl_id > 0 && idx_id > 0) {
                    cbm_edge_t e = {0};
                    e.project = project;
                    e.source_id = tbl_id;
                    e.target_id = idx_id;
            ...
            for (int f = 0; f < tbl->fk_count; f++) {
                cbm_db_fk_t *fk = &tbl->foreign_keys[f];
                total_fks++;

                /* Create REFERENCES_TABLE edge from this table to target table. */
                if (fk->target_table) {
                    /* Find target table node. */
                    char tgt_qn[CBM_SZ_1K];
                    const char *tgt_schema = fk->target_schema ? fk->target_schema : schema->name;
                    snprintf(tgt_qn, sizeof(tgt_qn), "db:%s.%s.%s",
                             db_node.name, tgt_schema, fk->target_table);
                    cbm_node_t tgt_node = {0};
                    if (cbm_store_find_node_by_qn(store, project, tgt_qn, &tgt_node) == 0 &&
                        tbl_id > 0 && tgt_node.id > 0) {
                        cbm_edge_t e = {0};
                        e.project = project;
                        e.source_id = tbl_id;
                        e.target_id = tgt_node.id;
                        e.type = "REFERENCES_TABLE";
                        char fk_props[CBM_SZ_512];
                        snprintf(fk_props, sizeof(fk_props),
                                 "{\"constraint\":\"%s\",\"on_delete\":\"%s\",\"on_update\":\"%s\"}",
                                 fk->constraint_name ? fk->constraint_name : "",
                                 fk->on_delete ? fk->on_delete : "",
                                 fk->on_update ? fk->on_update : "");
                        e.properties_json = fk_props;
                        cbm_store_insert_edge(store, &e);
                    }
                }
            }
    ...
    /* Store sample data as SampleData nodes. */
    for (int si = 0; si < result->sample_count; si++) {
        cbm_db_sample_t *sample = &result->samples[si];
        cbm_node_t sn = {0};
        sn.project = project;
        sn.name = sample->table_name;
        sn.label = "SampleData";
        char sn_qn[CBM_SZ_512];
        snprintf(sn_qn, sizeof(sn_qn), "db:%s.sample:%s",
                 result->database_name ? result->database_name : "db",
                 sample->table_name);
        sn.qualified_name = sn_qn;

        /* Store rows_json in properties. Keep it compact. */
        size_t json_len = sample->rows_json ? strlen(sample->rows_json) : 0;
        size_t props_sz = json_len + CBM_SZ_256;
        char *sn_props = cbm_malloc(props_sz);
        if (sn_props) {
            snprintf(sn_props, props_sz,
                     "{\"row_count\":%d,\"is_full_cache\":%s,\"is_encrypted\":%s,\"rows\":%s}",
                     sample->row_count,
                     sample->is_full_cache ? "true" : "false",
                     sample->is_encrypted ? "true" : "false",
                     sample->rows_json ? sample->rows_json : "[]");
            sn.properties_json = sn_props;
            cbm_store_upsert_node(store, &sn);
            cbm_free(sn_props);
        }
    }
    ...
    cbm_sb_t sb;
    cbm_sb_init(&sb);
    cbm_tree_scalar_str(&sb, "status", "indexed");
    cbm_tree_scalar_str(&sb, "database", result->database_name ? result->database_name : "unknown");
    cbm_tree_scalar_str(&sb, "dialect", cbm_db_dialect_label(result->dialect));
    cbm_tree_scalar_str(&sb, "version", result->version ? result->version : "unknown");
    cbm_tree_scalar_str(&sb, "source_env", cbm_db_source_env_label(result->source_env));
    cbm_tree_scalar_str(&sb, "detected_domain", result->domain_label ? result->domain_label : "Generic");
    cbm_tree_scalar_str(&sb, "seed_table", result->seed_table ? result->seed_table : "none");
    cbm_tree_scalar_int(&sb, "tables", total_tables);
    cbm_tree_scalar_int(&sb, "columns", total_columns);
    cbm_tree_scalar_int(&sb, "foreign_keys", total_fks);
    cbm_tree_scalar_int(&sb, "indexes", total_indexes);
    cbm_tree_scalar_int(&sb, "sample_tables", result->sample_count);

    char *text = cbm_sb_finish(&sb);
    cbm_db_result_free(result);
    free(project);

    char *res = cbm_mcp_text_result(text, false);
    free(text);
    return res;
}

/* ── handle_get_database_schema ───────────────────────────────── */

static char *handle_get_database_schema(cbm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    cbm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);

    char *table_pattern = cbm_mcp_get_string_arg(args, "table_pattern");
    char *schema_name = cbm_mcp_get_string_arg(args, "schema_name");
    char *detail = cbm_mcp_get_string_arg(args, "detail");
    char *format = cbm_mcp_get_string_arg(args, "format");

    bool want_json = format && strcmp(format, "json") == 0;
    (void)want_json;
    bool tables_only = detail && strcmp(detail, "tables_only") == 0;
    bool full_detail = detail && strcmp(detail, "full") == 0;

    /* Find all Table/View nodes in the project. */
    cbm_node_t *table_nodes = NULL;
    int table_count = 0;
    cbm_store_find_nodes_by_label(store, project, "Table", &table_nodes, &table_count);

    /* Also find Views. */
    cbm_node_t *view_nodes = NULL;
    int view_count = 0;
    cbm_store_find_nodes_by_label(store, project, "View", &view_nodes, &view_count);

    if (table_count == 0 && view_count == 0) {
        free(table_pattern);
        free(schema_name);
        free(detail);
        free(format);
        free(project);
        cbm_store_free_nodes(table_nodes, table_count);
        cbm_store_free_nodes(view_nodes, view_count);
        return cbm_mcp_text_result(
            "no database schema found for this project. Run index_database first.", true);
    }

    cbm_sb_t sb;
    cbm_sb_init(&sb);

    /* Merge table + view node arrays. */
    int all_count = table_count + view_count;
    cbm_tree_scalar_int(&sb, "tables_found", all_count);

    /* Emit each table. */
    for (int pass = 0; pass < CBM_SZ_2; pass++) {
        cbm_node_t *nodes = (pass == 0) ? table_nodes : view_nodes;
        int count = (pass == 0) ? table_count : view_count;

        for (int i = 0; i < count; i++) {
            const char *tname = nodes[i].name;
            if (!tname) {
                continue;
            }

            /* Apply table pattern filter. */
            if (table_pattern && table_pattern[0]) {
                if (!strstr(tname, table_pattern)) {
                    continue; /* simple substring match — regex TODO */
                }
            }

            char hdr[CBM_SZ_512];
            snprintf(hdr, sizeof(hdr), "\n%s (%s):\n", tname,
                     nodes[i].label ? nodes[i].label : "Table");
            cbm_sb_append(&sb, hdr);

            if (nodes[i].properties_json) {
                char prop_line[CBM_SZ_1K];
                snprintf(prop_line, sizeof(prop_line), "  properties: %s\n",
                         nodes[i].properties_json);
                cbm_sb_append(&sb, prop_line);
            }
            ...
            /* Find columns (edges from table). */
            cbm_edge_t *edges = NULL;
            int edge_count = 0;
            cbm_store_find_edges_by_source_type(store, nodes[i].id, "HAS_COLUMN",
                                                &edges, &edge_count);
            if (edge_count > 0) {
                cbm_sb_append(&sb, "  columns:\n");
                for (int e = 0; e < edge_count; e++) {
                    cbm_node_t col = {0};
                    if (cbm_store_find_node_by_id(store, edges[e].target_id, &col) == 0) {
                        char col_line[CBM_SZ_512];
                        if (full_detail && col.properties_json) {
                            snprintf(col_line, sizeof(col_line), "    - %s %s\n",
                                     col.name ? col.name : "?",
                                     col.properties_json);
                        } else {
                            snprintf(col_line, sizeof(col_line), "    - %s\n",
                                     col.name ? col.name : "?");
                        }
                        cbm_sb_append(&sb, col_line);
                        cbm_node_free_fields(&col);
                    }
                }
            }
            ...
            if (full_detail) {
                /* Find indexes. */
                cbm_edge_t *idx_edges = NULL;
                int idx_edge_count = 0;
                cbm_store_find_edges_by_source_type(store, nodes[i].id, "HAS_INDEX",
                                                    &idx_edges, &idx_edge_count);
                if (idx_edge_count > 0) {
                    cbm_sb_append(&sb, "  indexes:\n");
                    for (int e = 0; e < idx_edge_count; e++) {
                        cbm_node_t idx = {0};
                        if (cbm_store_find_node_by_id(store, idx_edges[e].target_id, &idx) == 0) {
                            char idx_line[CBM_SZ_512];
                            snprintf(idx_line, sizeof(idx_line), "    - %s %s\n",
                                     idx.name ? idx.name : "?",
                                     idx.properties_json ? idx.properties_json : "");
                            cbm_sb_append(&sb, idx_line);
                            cbm_node_free_fields(&idx);
                        }
                    }
                }
                cbm_store_free_edges(idx_edges, idx_edge_count);

                /* Find FK references. */
                cbm_edge_t *ref_edges = NULL;
                int ref_edge_count = 0;
                cbm_store_find_edges_by_source_type(store, nodes[i].id, "REFERENCES_TABLE",
                                                    &ref_edges, &ref_edge_count);
                if (ref_edge_count > 0) {
                    cbm_sb_append(&sb, "  references:\n");
                    for (int e = 0; e < ref_edge_count; e++) {
                        cbm_node_t ref = {0};
                        if (cbm_store_find_node_by_id(store, ref_edges[e].target_id, &ref) == 0) {
                            char ref_line[CBM_SZ_512];
                            snprintf(ref_line, sizeof(ref_line), "    -> %s %s\n",
                                     ref.name ? ref.name : "?",
                                     ref_edges[e].properties_json ? ref_edges[e].properties_json : "");
                            cbm_sb_append(&sb, ref_line);
                            cbm_node_free_fields(&ref);
                        }
                    }
                }
                cbm_store_free_edges(ref_edges, ref_edge_count);
            }
        }
    }
...
/* ── handle_generate_db_erd ───────────────────────────────────── */

static char *handle_generate_db_erd(cbm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    cbm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);

    char *detail_str = cbm_mcp_get_string_arg(args, "detail");
    int max_tables = cbm_mcp_get_int_arg(args, "max_tables", 30);

    /* Parse tables array if provided. */
    const char **filter_tables = NULL;
    int filter_count = 0;

    yyjson_doc *args_doc = yyjson_read(args, strlen(args), 0);
    if (args_doc) {
        yyjson_val *root = yyjson_doc_get_root(args_doc);
        yyjson_val *tbl_arr = yyjson_obj_get(root, "tables");
        if (tbl_arr && yyjson_is_arr(tbl_arr)) {
            filter_count = yyjson_arr_size(tbl_arr);
            if (filter_count > 0) {
                filter_tables = cbm_calloc((size_t)filter_count, sizeof(char *));
                yyjson_arr_iter iter;
                yyjson_arr_iter_init(tbl_arr, &iter);
                yyjson_val *v;
                int idx = 0;
                while ((v = yyjson_arr_iter_next(&iter)) != NULL && idx < filter_count) {
                    filter_tables[idx++] = yyjson_get_str(v);
                }
            }
        }
    }

    /* Reconstruct a cbm_db_result_t from the graph for ERD generation. */
    /* Find Database node. */
    cbm_node_t *db_nodes = NULL;
    int db_count = 0;
    cbm_store_find_nodes_by_label(store, project, "Database", &db_nodes, &db_count);

    if (db_count == 0) {
        cbm_store_free_nodes(db_nodes, db_count);
        if (args_doc) {
            yyjson_doc_free(args_doc);
        }
        cbm_free((void *)filter_tables);
        free(detail_str);
        free(project);
        return cbm_mcp_text_result(
            "no database indexed for this project. Run index_database first.", true);
    }

    /* Build a minimal cbm_db_result_t for the ERD generator. */
    cbm_db_result_t erd_result = {0};
    erd_result.database_name = (char *)(db_nodes[0].name ? db_nodes[0].name : "db");
    ...
    /* Build schema with tables. */
    erd_result.schema_count = 1;
    cbm_db_schema_t erd_schema = {0};
    erd_schema.name = (char *)"default";
    erd_schema.table_count = table_count;

    /* Allocate table array. */
    cbm_db_table_t *erd_tables = cbm_calloc(table_count > 0 ? (size_t)table_count : 1,
                                            sizeof(cbm_db_table_t));
    erd_schema.tables = erd_tables;

    for (int t = 0; t < table_count; t++) {
        erd_tables[t].name = (char *)table_nodes[t].name;

        /* Get columns. */
        cbm_edge_t *col_edges = NULL;
        int col_edge_count = 0;
        cbm_store_find_edges_by_source_type(store, table_nodes[t].id, "HAS_COLUMN",
                                            &col_edges, &col_edge_count);

        erd_tables[t].columns = cbm_calloc(col_edge_count > 0 ? (size_t)col_edge_count : 1,
                                           sizeof(cbm_db_column_t));
        erd_tables[t].column_count = 0;

        for (int e = 0; e < col_edge_count; e++) {
            cbm_node_t col = {0};
            if (cbm_store_find_node_by_id(store, col_edges[e].target_id, &col) == 0) {
                int ci = erd_tables[t].column_count;
                erd_tables[t].columns[ci].name = (char *)col.name;

                /* Parse properties for PK/FK/type info. */
                if (col.properties_json) {
                    yyjson_doc *pdoc = yyjson_read(col.properties_json,
                                                   strlen(col.properties_json), 0);
                    if (pdoc) {
                        yyjson_val *pr = yyjson_doc_get_root(pdoc);
                        yyjson_val *dt = yyjson_obj_get(pr, "data_type");
                        yyjson_val *pk = yyjson_obj_get(pr, "is_pk");
                        yyjson_val *fk = yyjson_obj_get(pr, "is_fk");
                        yyjson_val *uq = yyjson_obj_get(pr, "is_unique");
                        yyjson_val *nl = yyjson_obj_get(pr, "is_nullable");

                        if (dt) {
                            erd_tables[t].columns[ci].data_type = heap_strdup(yyjson_get_str(dt));
                        }
                        erd_tables[t].columns[ci].is_pk = pk ? yyjson_get_bool(pk) : false;
                        erd_tables[t].columns[ci].is_fk = fk ? yyjson_get_bool(fk) : false;
                        erd_tables[t].columns[ci].is_unique = uq ? yyjson_get_bool(uq) : false;
                        erd_tables[t].columns[ci].is_nullable = nl ? yyjson_get_bool(nl) : true;
                        yyjson_doc_free(pdoc);
                    }
                }
                erd_tables[t].column_count = ci + 1;
                /* Don't free col fields — they're from the store and referenced by erd_tables. */
            }
        }
        cbm_store_free_edges(col_edges, col_edge_count);

        /* Get FK references. */
        cbm_edge_t *ref_edges = NULL;
        int ref_edge_count = 0;
        cbm_store_find_edges_by_source_type(store, table_nodes[t].id, "REFERENCES_TABLE",
                                            &ref_edges, &ref_edge_count);

        erd_tables[t].foreign_keys = cbm_calloc(
            ref_edge_count > 0 ? (size_t)ref_edge_count : 1, sizeof(cbm_db_fk_t));
        erd_tables[t].fk_count = 0;

        for (int e = 0; e < ref_edge_count; e++) {
            cbm_node_t ref = {0};
            if (cbm_store_find_node_by_id(store, ref_edges[e].target_id, &ref) == 0) {
                int fi = erd_tables[t].fk_count;
                erd_tables[t].foreign_keys[fi].target_table = (char *)ref.name;
                erd_tables[t].foreign_keys[fi].constraint_name = (char *)"";
                /* Find which column is the FK source. */
                for (int c = 0; c < erd_tables[t].column_count; c++) {
                    if (erd_tables[t].columns[c].is_fk) {
                        erd_tables[t].foreign_keys[fi].source_columns =
                            cbm_calloc(1, sizeof(char *));
                        erd_tables[t].foreign_keys[fi].source_columns[0] =
                            (char *)erd_tables[t].columns[c].name;
                        erd_tables[t].foreign_keys[fi].source_column_count = 1;
                        break;
                    }
                }
                erd_tables[t].fk_count = fi + 1;
            }
        }
        cbm_store_free_edges(ref_edges, ref_edge_count);
    }

    erd_result.schemas = &erd_schema;

    /* Parse detail. */
    cbm_erd_detail_t erd_detail = CBM_ERD_COLUMNS;
    if (detail_str) {
        cbm_erd_parse_detail(detail_str, &erd_detail);
    }

    /* Generate Mermaid. */
    char *mermaid = cbm_erd_generate_mermaid(&erd_result, filter_tables, filter_count,
                                             erd_detail, max_tables);

    /* Cleanup — careful not to double-free store-owned strings. */
    for (int t = 0; t < table_count; t++) {
        for (int c = 0; c < erd_tables[t].column_count; c++) {
            free(erd_tables[t].columns[c].data_type);
        }
        cbm_free(erd_tables[t].columns);
        /* FK source_columns arrays were cbm_calloc'd by us. */
        for (int f = 0; f < erd_tables[t].fk_count; f++) {
            cbm_free(erd_tables[t].foreign_keys[f].source_columns);
        }
        cbm_free(erd_tables[t].foreign_keys);
    }
    cbm_free(erd_tables);
    cbm_store_free_nodes(db_nodes, db_count);
    cbm_store_free_nodes(table_nodes, table_count);
    ...
    if (!mermaid) {
        free(project);
        return cbm_mcp_text_result("failed to generate ERD", true);
    }

    char *res = cbm_mcp_text_result(mermaid, false);
    free(mermaid);
    free(project);
    return res;
}

/* ── handle_get_table_data ────────────────────────────────────── */

static char *handle_get_table_data(cbm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    cbm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);

    char *table_name = cbm_mcp_get_string_arg(args, "table_name");
    if (!table_name) {
        free(project);
        return cbm_mcp_text_result("missing required argument: table_name", true);
    }

    /* Find the SampleData node for this table. */
    cbm_node_t *sample_nodes = NULL;
    int sample_count = 0;
    cbm_store_find_nodes_by_label(store, project, "SampleData", &sample_nodes, &sample_count);

    char *found_json = NULL;
    for (int i = 0; i < sample_count; i++) {
        if (sample_nodes[i].name && strcmp(sample_nodes[i].name, table_name) == 0) {
            if (sample_nodes[i].properties_json) {
                found_json = heap_strdup(sample_nodes[i].properties_json);
            }
            break;
        }
    }

    cbm_store_free_nodes(sample_nodes, sample_count);

    if (!found_json) {
        char err[CBM_SZ_512];
        snprintf(err, sizeof(err),
                 "no sample data cached for table '%s'. Run index_database first.", table_name);
        free(table_name);
        free(project);
        return cbm_mcp_text_result(err, true);
    }
    ...
    free(table_name);
    free(project);
    char *res = cbm_mcp_text_result(found_json, false);
    free(found_json);
    return res;
}

/* ── handle_search_database_graph ─────────────────────────────── */

static char *handle_search_database_graph(cbm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    cbm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
    ...
    char *query = cbm_mcp_get_string_arg(args, "query");
    char *name_pattern = cbm_mcp_get_string_arg(args, "name_pattern");
    char *entity_type = cbm_mcp_get_string_arg(args, "entity_type");
    int limit = cbm_mcp_get_int_arg(args, "limit", 50);
    int offset = cbm_mcp_get_int_arg(args, "offset", 0);

    /* Determine which labels to search. */
    const char *labels[] = {"Table", "Column", "Index", "Database", "Schema", "View"};
    int label_count = 6;

    if (entity_type) {
        if (strcmp(entity_type, "table") == 0) {
            labels[0] = "Table";
            label_count = 1;
        } else if (strcmp(entity_type, "column") == 0) {
            labels[0] = "Column";
            label_count = 1;
        } else if (strcmp(entity_type, "index") == 0) {
            labels[0] = "Index";
            label_count = 1;
        } else if (strcmp(entity_type, "foreign_key") == 0) {
            /* FK info is on REFERENCES_TABLE edges; search Table nodes with FK edges. */
            labels[0] = "Table";
            label_count = 1;
        }
    }

    /* Use search_graph with the appropriate label filter. */
    const char *search_term = query ? query : name_pattern;
    ...
    for (int l = 0; l < label_count; l++) {
        cbm_node_t *nodes = NULL;
        int count = 0;
        cbm_store_find_nodes_by_label(store, project, labels[l], &nodes, &count);

        for (int i = 0; i < count; i++) {
            /* Apply search filter. */
            if (search_term && search_term[0]) {
                bool match = false;
                if (nodes[i].name && strcasestr_compat(nodes[i].name, search_term)) {
                    match = true;
                }
                if (!match && nodes[i].qualified_name &&
                    strcasestr_compat(nodes[i].qualified_name, search_term)) {
                    match = true;
                }
                if (!match) {
                    continue;
    ...
    /* Header. */
    cbm_sb_t result_sb;
    cbm_sb_init(&result_sb);
    cbm_tree_scalar_int(&result_sb, "total", total);
    cbm_tree_scalar_int(&result_sb, "shown", shown);
    cbm_tree_scalar_int(&result_sb, "offset", offset);
    cbm_tree_scalar_bool(&result_sb, "has_more", total > offset + shown);
    cbm_sb_append(&result_sb, "\n");

    char *body = cbm_sb_finish(&sb);
    if (body) {
        cbm_sb_append(&result_sb, body);
        free(body);
    }
    ...
    char *text = cbm_sb_finish(&result_sb);
    char *res = cbm_mcp_text_result(text, false);
    free(text);
    return res;
}

/* A simple case-insensitive strstr for cross-platform compatibility. */
static const char *strcasestr_compat(const char *haystack, const char *needle) {
    if (!haystack || !needle) {
        return NULL;
    }
...
static char *dispatch_tool(cbm_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    ...
    /* Database tools */
    if (strcmp(tool_name, "index_database") == 0) {
        return handle_index_database(srv, args_json);
    }
    if (strcmp(tool_name, "get_database_schema") == 0) {
        return handle_get_database_schema(srv, args_json);
    }
    if (strcmp(tool_name, "generate_db_erd") == 0) {
        return handle_generate_db_erd(srv, args_json);
    }
    if (strcmp(tool_name, "get_table_data") == 0) {
        return handle_get_table_data(srv, args_json);
    }
    if (strcmp(tool_name, "search_database_graph") == 0) {
        return handle_search_database_graph(srv, args_json);
    }
    char msg[CBM_SZ_256];
