...
**The fastest and most efficient full-stack code and database intelligence engine for AI coding agents.** Formerly `codebase-memory-mcp`, now unifying code graphs with live/local databases (PostgreSQL, MySQL, SQLite, MongoDB) into one offline graph. Full-indexes an average repository in milliseconds, the Linux kernel (28M LOC, 75K files) in 3 minutes. Answers structural and schema queries in under 1ms. This npm wrapper downloads, verifies, and caches the selected native runtime set: the executable, its authenticated integration asset, and—when requested—the content-addressed UI pack.

High-quality parsing through [tree-sitter](https://tree-sitter.github.io/tree-sitter/) AST analysis across 158 languages and native database introspection across 4 database engines — producing a persistent knowledge graph of functions, classes, call chains, HTTP routes, tables, ERDs, foreign keys, indexes, and referentially coherent sample data. 20 MCP tools. No hosted service or API key; this package requires Node.js to manage and launch the cached native runtime set. Plug and play across 43 automatic/conditional client surfaces.
...
- **20 MCP tools** — search, trace, architecture, database introspection, ERDs, table sampling, impact analysis, Cypher queries, dead code detection, cross-service HTTP linking, ADR management, and more.
