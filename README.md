# Full Stack Application Memory MCP

[![GitHub Release](https://img.shields.io/github/v/release/DeusData/codebase-memory-mcp?style=flat&color=blue)](https://github.com/DeusData/codebase-memory-mcp/releases/latest)
[![License](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![CI](https://img.shields.io/github/actions/workflow/status/DeusData/codebase-memory-mcp/dry-run.yml?label=CI)](https://github.com/DeusData/codebase-memory-mcp/actions/workflows/dry-run.yml)
[![Tests](https://img.shields.io/badge/tests-6782_passing-brightgreen)](https://github.com/DeusData/codebase-memory-mcp)
[![Languages](https://img.shields.io/badge/languages-158-orange)](https://github.com/DeusData/codebase-memory-mcp)
[![Databases](https://img.shields.io/badge/databases-PostgreSQL_%7C_MySQL_%7C_SQLite_%7C_MongoDB-blue)](#database-knowledge-graph)
...
**The fastest and most efficient full-stack code and database intelligence engine for AI coding agents.** Formerly `codebase-memory-mcp`, now upgraded to unify application source code with live/local backend databases into one offline knowledge graph. Full-indexes an average repository in milliseconds, the Linux kernel (28M LOC, 75K files) in 3 minutes, and complete multi-tenant database schemas with coherent foreign-key sample sets in seconds. Answers structural and schema queries in under 1ms. Ships as a native executable with a small verified runtime-asset set for macOS, Linux, and Windows — download, run `install`, done.

High-quality parsing through [tree-sitter](https://tree-sitter.github.io/tree-sitter/) AST analysis across all 158 languages, enhanced with [**Hybrid LSP** semantic type resolution](#hybrid-lsp) (Python, TypeScript / JavaScript / JSX / TSX, PHP, C#, Go, C, C++, Java, Kotlin, Rust, Perl) and [**Database Knowledge Graph**](#database-knowledge-graph) (PostgreSQL, MySQL, SQLite, MongoDB) — producing a persistent knowledge graph of functions, classes, call chains, HTTP routes, database tables, ERDs, foreign keys, indexes, and referentially coherent sample data. 20 MCP tools. No language runtime, hosted service, or API key. Plug and play across 43 supported automatic/conditional client surfaces.

> **Research** — The design and benchmarks behind this project are described in the preprint [*Codebase-Memory: Tree-Sitter-Based Knowledge Graphs for LLM Code Exploration via MCP*](https://arxiv.org/abs/2603.27277) (arXiv:2603.27277). Evaluated across 31 real-world repositories: 83% answer quality, 10× fewer tokens, 2.1× fewer tool calls vs. file-by-file exploration.

> **Security & Trust** — This tool reads your codebase and writes to your agent configuration files. That is what it is designed to do. If you prefer to audit before running, the [full source is here](https://github.com/DeusData/codebase-memory-mcp). For each release product, three behaviourally identical executable candidates (unstripped, debug-stripped, stripped) are submitted to VirusTotal before testing; the selected candidate is then packaged with its SHA-256 unchanged. Release notes link every measured candidate result. Publication permits only the narrowly documented single-Microsoft `!ml` tolerance in [SECURITY.md](SECURITY.md#our-release-policy). All processing happens 100% locally; your code and database data never leave your machine. Found a security issue? We want to know — see [SECURITY.md](SECURITY.md). Security is Priority #1 for us.

<p align="center">
  <img src="docs/graph-ui-screenshot.png" alt="Graph visualization UI showing the Full Stack Application Memory MCP knowledge graph" width="800">
  <br>
  <em>Built-in 3D graph visualization — explore your code + database knowledge graph at localhost:9749</em>
</p>

## Why Full Stack Application Memory MCP

- **Full-Stack Context** — Bridges code (functions, classes, models, routes) with databases (tables, columns, foreign keys, indexes, views, and referentially coherent sample data).
- **Extreme indexing speed** — Linux kernel (28M LOC, 75K files) in 3 minutes. RAM-first pipeline: LZ4 compression, in-memory SQLite, fused Aho-Corasick pattern matching. Memory released after indexing.
- **Zero Persistent DB Connections** — Introspects databases once, builds a compressed local graph representation, and enables 100% offline queries for AI agents.
- **Domain-Aware Coherent Sampling** — Automatically fingerprints the application domain (E-commerce, CMS, SaaS, IAM, Healthcare) and traverses foreign-key chains to provide referentially consistent sample rows.
...
- **158 languages & 4 DB engines** — vendored tree-sitter grammars + native/CLI database bridges (Postgres, MySQL, SQLite, MongoDB).
- **120x fewer tokens** — 5 structural queries: ~3,400 tokens vs ~412,000 via file-by-file search. One graph query replaces dozens of grep/read cycles.
- **43 supported automatic/conditional client surfaces** — `install` configures detected clients and safely activates conditional clients only when their documented platform, marker, or explicit existing config path is present. See [Multi-Agent Support](#multi-agent-support) for the complete matrix.
- **Built-in graph visualization** — 3D interactive UI at `localhost:9749`, served from the binary itself.
- **Instant Mermaid ERD Generation** — Export complete visual Entity-Relationship Diagrams with cardinality and key constraints on-demand.
- **20 MCP tools** — search, trace, architecture, database introspection, ERDs, table sampling, impact analysis, targeted index-coverage checks, Cypher queries, dead code detection, cross-service HTTP linking, ADR management, and more.
...
### Database Knowledge Graph (SQL & NoSQL)

| Tool | Description |
|------|-------------|
| `index_database` | Introspect live or local databases (PostgreSQL, MySQL, SQLite, MongoDB) **once**, extract schema metadata (tables, columns, PKs, FKs, indexes, comments) and coherent sample rows into the local graph. Zero persistent connections. |
| `get_database_schema` | Query cached database schemas, table definitions, column types, and constraints offline without connecting to the DB. |
| `generate_db_erd` | Generate visual Mermaid `erDiagram` syntax directly from graph metadata with cardinality and key constraints. Supports `keys_only`, `columns`, and `full` detail levels. |
| `get_table_data` | Retrieve referentially consistent sample rows or complete reference/lookup tables with automatic sensitive column masking. |
| `search_database_graph` | Search across tables, columns, indexes, and foreign-key relationships by keyword or regex pattern. |
...
**Codebase**: `Project`, `Package`, `Folder`, `File`, `Module`, `Class`, `Function`, `Method`, `Interface`, `Enum`, `Type`, `Route`, `Resource`  
**Database**: `Database`, `Schema`, `Table`, `Column`, `Index`, `ForeignKey`, `SampleData`

### Edge Types

**Code Structure**: `CONTAINS_PACKAGE`, `CONTAINS_FOLDER`, `CONTAINS_FILE`, `DEFINES`, `DEFINES_METHOD`, `IMPORTS`, `CALLS`, `CALL_REFERENCE`, `HTTP_CALLS`, `ASYNC_CALLS`, `IMPLEMENTS`, `HANDLES`, `USAGE`, `CONFIGURES`, `WRITES`, `MEMBER_OF`, `TESTS`, `USES_TYPE`, `FILE_CHANGES_WITH`  
**Database Schema**: `CONTAINS_SCHEMA`, `CONTAINS_TABLE`, `HAS_COLUMN`, `HAS_INDEX`, `INDEXES_COLUMN`, `FOREIGN_KEY_TO`, `REFERENCES_TABLE`  
**Full-Stack Cross-Links**: `READS_TABLE`, `WRITES_TABLE`, `DEFINES_TABLE`
...
**Full-stack intelligence: bridging source code with SQL and NoSQL databases.**

AI coding agents often struggle with data layer context: What columns exist on `users`? How does `orders` join to `order_items`? What is the schema of the MongoDB `events` collection? Which database tables are lookup tables?

**Full Stack Application Memory MCP** introduces native database introspection for PostgreSQL, MySQL, SQLite, and MongoDB — converting database schemas into graph nodes and foreign-key relationships stored directly in the local offline graph.

```mermaid
graph TD
    subgraph "Target Databases (Local or Remote)"
        PG[(PostgreSQL)]
        MY[(MySQL / MariaDB)]
        SQ[(SQLite)]
        MO[(MongoDB)]
    ...
    subgraph "Full Stack Application Memory MCP"
        DISP[Introspection Engine<br/><code>db_introspect.c</code>]
        DOM[Domain Detection &<br/>Heuristics Classifier]
        SAMPLER[Coherent FK-Chain Sampler<br/>& Sensitive Masking]
        ERD[Mermaid ERD Engine<br/><code>erd_generator.c</code>]
    end

    subgraph "Local Graph (.codebase-memory/)"
        GRAPH[(Offline Unified Store)]
        DB_NODE[Database & Schema Nodes]
        TBL_NODE[Table & View Nodes]
        COL_NODE[Column & Index Nodes]
        SAMPLE_NODE[Referentially Coherent Sample Data]
        CODE_NODE[Code Functions & Models]
    end

    PG -->|One-time CLI bridge| DISP
    MY -->|One-time CLI bridge| DISP
    SQ -->|One-time Native lib| DISP
    MO -->|One-time CLI bridge| DISP
...
1. **One-Time Introspection, Zero Standing DB Connections**:
   - `index_database` connects **once** to extract tables, views, columns, PKs, FKs, indexes, comments, and coherent sample rows.
   - All subsequent queries by AI coding agents hit the **local offline graph** with sub-millisecond response times.
2. **Domain-Aware Coherent Sampling**:
   - Automatically fingerprints application domains (E-commerce, Publishing/CMS, SaaS Multi-tenant, Auth/IAM, Healthcare) from schema patterns.
   - Traverses foreign-key chains from a domain seed table (e.g. `orders` → `order_items` → `products` → `customers`) to store referentially consistent sample rows rather than isolated, broken fragments.
   - Reference/lookup tables (`*_types`, `*_roles`, `*_status`, `permissions`, `< 100` rows) are cached **in full**.
3. **Sensitive Column Masking & Security**:
   - Columns matching `*password*`, `*secret*`, `*token*`, `*ssn*`, `*credit_card*`, `*auth*`, `*api_key*` are automatically masked as `"***REDACTED***"` before storage.
   - Database credentials and live connection strings are **never persisted** in the graph; only the source environment label (`production`, `staging`, `local`, `development`) is recorded.
4. **Mermaid ERD Generation on Demand**:
   - `generate_db_erd` outputs ready-to-render Mermaid `erDiagram` syntax directly from graph metadata.
   - Supports `keys_only`, `columns`, and `full` detail levels, table filters, and `max_tables` overflow guards.
